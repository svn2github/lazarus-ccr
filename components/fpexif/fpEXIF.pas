...
uses
 ...
 {$ENDIF}
  fpeGlobal, fpeUtils, fpeTags, fpeIptc;
...
//  Added the following elements to make the structure a little more code-friendly
  ...
  public
    constructor Create(AParent: TObject; ABigEndian: Boolean;
      ABuildCode: integer = GenAll);
    procedure Assign(source: TImageInfo);
    destructor Destroy; override;

    // Reader interface
    procedure AddTagFromReader(ATag: TTagEntry);
    procedure AddThumbnailFromReader(ABuffer: TBytes);
...
constructor TImageInfo.Create(AParent: TObject; ABigEndian: Boolean;
  ABuildCode: integer = GenAll);
begin
  inherited Create;
  FParent := AParent;
  MotorolaOrder := ABigEndian;
  LoadTagDescs(True);  // initialize global structures
  FITagCount := 0;
  BuildList := ABuildCode;
  ClearDirStack;
end;
...
// To be called by the reader.
procedure TImageInfo.AddTagFromReader(ATag: TTagEntry);
begin
  ATag.Data := InternalGetTagValueAsString(ATag);
  if ATag.ParentID = 1 then
    SetThumbTagByID(ATag.Tag, ATag)
  else
    SetTagByID(ATag.Tag, ATag);
end;

// To be called by the reader.
procedure TImageInfo.AddThumbnailFromReader(ABuffer: TBytes);
begin
  SetLength(FThumbnailBuffer, Length(ABuffer));
  if Length(ABuffer) > 0 then
    Move(ABuffer[0], FThumbnailBuffer[0], Length(ABuffer));
end;
